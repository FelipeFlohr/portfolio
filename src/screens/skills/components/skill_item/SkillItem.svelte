...
<template>
    <div class={`flex flex-col justify-center items-center ${tech.isLarge ? "w-32" : "w-20"} h-24 rounded-md shadow-md transition-transform hover:scale-110 hover:cursor-default ${tech.whiteForeground ? "" : "black-border"}`} style={`background-color: ${tech.backgroundColor}; color: ${tech.whiteForeground ? "#FFF" : "#000"}`}>
        <div>
